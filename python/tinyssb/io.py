...
class UDP_MULTICAST(FACE):
    ...
    def __init__(self, addr):
        ...
        self.snd_sock.bind(mk_addr("192.168.178.52",0)) #edit to bind to specific net interface
        if sys.implementation.name != 'micropython':
            self.snd_sock.setsockopt(socket.IPPROTO_IP,
                                     socket.IP_MULTICAST_TTL, 2)
        ...
        try:
            ...
            if sys.implementation.name=='micropython' and \
                ...
                if sys.platform == "win32":
                    mreq = struct.pack('4s4s', socket.inet_aton(addr[0]), socket.inet_aton("192.168.178.52")) #4s4s, socket.inet_aton for specific network interface, 4s4s socket.INADDR_ANY
